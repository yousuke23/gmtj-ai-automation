...
当面の最優先スプリント（合意済み）。**このリポでナレッジを厚くする作業順は 14 → 01 → 18**（[docs/README.md](docs/README.md)）。
...
**運用方針（2026-05）:** **Open WebUI / Docker は当面使わずに進める。** `kb/`・`tour/`・`marketing/`・`eval/`・`make ci` を正とし、`deploy/` の手順は必要になったときだけ参照する。
...
2. `kb/intents.yaml` と `kb/templates/{intent}.{ja,en,ko}.md` の最小セット（`scripts/verify-kb-templates.sh` で検証。`make ci` でゴールデン件数も確認）  
3. `eval/golden-questions.md`（英日韓混在のゴールデン質問セット。版・問数はヘッダ参照）  
4. `deploy/README.md` + `docker-compose.yml` + **[QUICKSTART-サービス開始.md](deploy/QUICKSTART-サービス開始.md)**（**社内限定**のRAG/チャットMVP用。**Open WebUI／Docker を使うチームのみ**。公開前に人間承認）  
