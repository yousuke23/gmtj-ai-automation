...
**運用方針:** **ナレッジの正本は `kb/`・`tour/`・`marketing/`・`eval/`**（Git で管理）。**Open WebUI** はその `kb` を **社内パイロット用チャット**で試す窓口として利用する。起動・ナレッジ・共有前ゲートは **`deploy/QUICKSTART-サービス開始.md`** → **`deploy/SERVICE-LAUNCH.md`**。Anthropic 接続のコツは **`deploy/openwebui-anthropic-connection.md`**。Docker を使わない日は **`make kb-zip`** だけでも ZIP は作れる。  
**デスクトップの `GMTJ OS` フォルダ内 PDF（OS3_1〜10）のパス一覧・運用メモ:** **`docs/GMTJ-OS-PDF-インデックス.md`**（`make verify-gmtj-os-pdfs` で存在確認）。
