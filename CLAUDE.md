...
当面の最優先スプリント（合意済み）。**このリポでのナレッジ整備の進め方は 14 → 01 → 18**（一覧は [docs/README.md](docs/README.md)）。

1. **14 Regional Music Alliance** — 全国＋インバウンドの **AI観光コンシェルジュ／ナビ**（問い合わせ一次対応・案内）  
2. **01 Global Music Tour** — 英語インバウンド向けの「第一のツアー」商品（ルート・体験・安全・マナー）  
3. **18 Global Digital Marketing** — 全事業横断の集客・コンテンツ・提案型マーケの基盤  

21事業の対応表・リポの入口: [`docs/business-map.md`](docs/business-map.md)。
...
2. `kb/intents.yaml` と `kb/templates/{intent}.{ja,en,ko}.md` の最小セット（韓国語は `ko` を `scripts/verify-kb-templates.sh` で検証。`make ci` でゴールデン件数も確認）  
...
5. [`docs/business-map.md`](docs/business-map.md) — 21事業1行サマリ **および** 01/14/18 のリポ入口
...
次を満たしたら **当面スプリント完了** とみなす（運用で拡張可）。

### 全体ゲート

- ルートで **`make ci`** が通る（`kb` テンプレ整合＋ゴールデン質問件数）。  
- [`docs/business-map.md`](docs/business-map.md) に **01 / 14 / 18 のリポ入口**が記載されている。

### 14（コンシェルジュ）

- [`eval/golden-questions.md`](eval/golden-questions.md) のセットに対し、[`kb/policy.md`](kb/policy.md)・[`kb/templates/`](kb/templates/) で **一貫した返答方針**がある。  
- [`kb/intents.yaml`](kb/intents.yaml) とテンプレファイル名が対応している。

### 01（ツアー）

- [`tour/`](tour/) に英語ゲスト向け v0.1・スタッフ Runbook v0.1・[`checklist-packing-izu-san.md`](tour/checklist-packing-izu-san.md) がある。  
- [`kb/tours/`](kb/tours/) に FAQ・安全メモが **14 へ供給**できる形である。

### 18（マーケ）

- [`marketing/90day-ideas.md`](marketing/90day-ideas.md) と [`marketing/content-pillars.md`](marketing/content-pillars.md) があり、[`kb/brand/`](kb/brand/) と **表記・柱が整合**している。

### 任意（社内チャット MVP）

- Open WebUI を使う場合のみ [`deploy/SERVICE-LAUNCH.md`](deploy/SERVICE-LAUNCH.md) のゲートを満たす。**Docker なしでも上記は完了しうる。**
